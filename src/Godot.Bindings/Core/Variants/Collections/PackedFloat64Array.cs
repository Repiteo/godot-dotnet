    ...
    IDisposable
{
    ...
    internal ReadOnlySpan<double> AsReadOnlySpan()
    {
        return NativeValue.DangerousSelfRef.AsReadOnlySpan();
    }

    internal ReadOnlySpan<double> AsReadOnlySpan(int start)
    {
        return AsReadOnlySpan().Slice(start);
    }

    internal ReadOnlySpan<double> AsReadOnlySpan(int start, int length)
    {
        return AsReadOnlySpan().Slice(start, length);
    }

    internal ReadOnlySpan<double> AsReadOnlySpan(Index startIndex)
    {
        int actualIndex = startIndex.GetOffset(Count);
        return AsReadOnlySpan().Slice(actualIndex);
    }

    internal ReadOnlySpan<double> AsReadOnlySpan(Range range)
    {
        (int start, int length) = range.GetOffsetAndLength(Count);
        return AsReadOnlySpan().Slice(start, length);
    }
