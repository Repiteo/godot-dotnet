    ...
    IDisposable
{
    ...
    internal ReadOnlySpan<float> AsReadOnlySpan()
    {
        return NativeValue.DangerousSelfRef.AsReadOnlySpan();
    }

    internal ReadOnlySpan<float> AsReadOnlySpan(int start)
    {
        return AsReadOnlySpan().Slice(start);
    }

    internal ReadOnlySpan<float> AsReadOnlySpan(int start, int length)
    {
        return AsReadOnlySpan().Slice(start, length);
    }

    internal ReadOnlySpan<float> AsReadOnlySpan(Index startIndex)
    {
        int actualIndex = startIndex.GetOffset(Count);
        return AsReadOnlySpan().Slice(actualIndex);
    }

    internal ReadOnlySpan<float> AsReadOnlySpan(Range range)
    {
        (int start, int length) = range.GetOffsetAndLength(Count);
        return AsReadOnlySpan().Slice(start, length);
    }
