    ...
    IDisposable
{
    ...
    internal ReadOnlySpan<Vector2> AsReadOnlySpan()
    {
        return NativeValue.DangerousSelfRef.AsReadOnlySpan();
    }

    internal ReadOnlySpan<Vector2> AsReadOnlySpan(int start)
    {
        return AsReadOnlySpan().Slice(start);
    }

    internal ReadOnlySpan<Vector2> AsReadOnlySpan(int start, int length)
    {
        return AsReadOnlySpan().Slice(start, length);
    }

    internal ReadOnlySpan<Vector2> AsReadOnlySpan(Index startIndex)
    {
        int actualIndex = startIndex.GetOffset(Count);
        return AsReadOnlySpan().Slice(actualIndex);
    }

    internal ReadOnlySpan<Vector2> AsReadOnlySpan(Range range)
    {
        (int start, int length) = range.GetOffsetAndLength(Count);
        return AsReadOnlySpan().Slice(start, length);
    }
